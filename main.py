...
if vivi_file:
    ...
    try:
        ...
        # Selecionamos videoId e tentamos pegar metadados básicos se existirem
        query = """
            SELECT DISTINCT s.songId, s.title, s.artist, s.duration, s.thumbnailUrl 
            FROM playlist_song_map m
            LEFT JOIN song s ON m.songId = s.songId
            ORDER BY m.rowid
        """
        df_songs = pd.read_sql_query(query, conn_v)
        conn_v.close()

        lista_ids = df_songs['songId'].tolist()
        st.success(f"✅ {len(lista_ids)} músicas encontradas.")

        if not os.path.exists(BASE_SIMP) or not os.path.exists(SETTINGS_FILE):
            st.error("Arquivos base (simpmusic.db ou settings) ausentes na raiz.")
        else:
            db_output_path = os.path.join(proc_dir, "Music Database")
            ...
            try:
                ...
                # --- 1. POPULAR TABELA SONG COM METADADOS ---
                # Isso evita que a lista apareça vazia no app
                for _, row in df_songs.iterrows():
                    cursor.execute("""
                        INSERT OR REPLACE INTO song (videoId, title, artist, duration, thumbnailUrl) 
                        VALUES (?, ?, ?, ?, ?)
                    """, (row['songId'], row['title'], row['artist'], row['duration'], row['thumbnailUrl']))

                # --- 2. ATUALIZAR MAPA DA PLAYLIST ---
                ...
                # --- 3. ATUALIZAR COLUNA TRACKS (JSON SEM ESPAÇOS) ---
                ...
                with zipfile.ZipFile(final_backup_path, 'w') as zipf:
                    zipf.write(db_output_path, arcname="Music Database")
                    zipf.write(SETTINGS_FILE, arcname=SETTINGS_FILE)

                with open(final_backup_path, "rb") as f:
                    st.download_button(
                        label="📥 BAIXAR SIMPMUSIC.BACKUP CORRIGIDO",
                        data=f.read(),
